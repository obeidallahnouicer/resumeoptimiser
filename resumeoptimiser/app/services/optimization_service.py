"""OptimizationService – orchestrates the full 8-step pipeline.
...
Pipeline:
  ...
  4. ScoreExplainerAgent   → ExplanationReportSchema (LLM)
  5. CVRewriteAgent        → OptimizedCVSchema       (LLM)
  6. CVValidatorAgent      → CVValidatorOutput       (rules only)
  7. RescoreAgent          → ImprovedScoreSchema     (embeddings only)
  8. ReportGeneratorAgent  → ComparisonReportSchema  (LLM)
"""
...
class OptimizationService:
    ...
    def run(self, cv_text: str, job_text: str) -> ComparisonReportSchema:
        ...
        Args:
            cv_text: Raw CV text (extracted from file upload).
            job_text: Raw job description text.

        Returns:
            ComparisonReportSchema with scores, explanation, and rewritten CV.

        Raises:
            AgentExecutionError: If any pipeline step fails.
            ValidationError: If the optimised CV fails business rule checks.
        """
    ...
    # ------------------------------------------------------------------
    # Private step wrappers – each wraps one agent call
    # ------------------------------------------------------------------
    ...
    def _score(self, cv, job):
        return self._matcher.execute(SemanticMatcherInput(cv=cv, job=job))
